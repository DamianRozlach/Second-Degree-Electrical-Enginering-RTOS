...
int counter = 0;
( void ) pvParameters;
const char *wiadomosc="DamianRozlach";
...
for ( ; ; )
{
xQueueSend(xQueue, &wiadomosc[counter], oczekiwanie_kolejka);
counter++;
if(counter==13){
counter=0;
}
...
for ( ; ; )
{
xQueueReceive(xQueue, &Odbior, oczekiwanie_kolejka);
printf("Zmienna odebrana = %c\n",Odbior);
vTaskDelay(opoznienie);
}
}
int main(void)
...
xQueue = xQueueCreate(ilosc_elementow, sizeof(char));
if (xQueue != NULL)
{
